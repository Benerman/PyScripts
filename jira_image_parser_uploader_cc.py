...
def export_csv(browser):
    csv_path = Path(os.getcwd() + '/Downloads/No Attachments _ 1 month (JIRA).csv')
    ...
    if not os.access(csv_path, os.R_OK):
        ...
        print('Path searching for CSV file: {}'.format(csv_path))
        sys.exit()
    return csv_path
...
def image_downloader(image_url, current_path):
    # print(image_url)
    file_name = image_url.split('/')[-1]
    # print(file_name)
    file_path = Path(current_path + '/tmp/')
    # print(file_path)
    ...
    try:
        ...
        with open(full_path, 'wb') as imagefile:
            ...
            filenamecount += 1
            print('Downloaded file {} to {}'.format(file_name, file_path))
            return True
    except:
        # filename = filename + str(filenamecount)
        print('{}[{}].{}'.format(Path(file_path, file_name.split('.')[0]), filenamecount, file_name.split('.')[-1]))
        with open('{}[{}].{}'.format(Path(file_path, file_name.split('.')[0]), filenamecount, file_name.split('.')[-1]), 'wb') as imagefile:
            imagefile.write(urllib2.urlopen(image_url).read())
            filenamecount += 1
            print('--Downloaded file {} to {}'.format(file_name, file_path))
            return True
...
def send_img_link(browser, image_link):
    file_path = Path(os.getcwd() + '/' + image_link.split('/')[-1])
    pyautogui.typewrite(str(file_path))
    print('Sent {} to file browser'.format(file_path))
...
def upload_image_to_jira(site, browser, image_link, pwd):
    ...
    if  upload_exist_check(browser, image_link):
        return False
    try:
        dload = image_downloader(image_link, pwd)
        if dload:
...
def main():
    ...
    for jira, url in urls:
        ...
        upload_image_to_jira(jira_url, browser, url_img_link, pwd)
        row_count += 1
    browser.close()
    print('Done')
