...
with open(r'processed_audits\\' + today.strftime('%Y%m%d') + '\\' + output_csv, mode='wb') as csv_output:
	...
	with open(csv_input_file, mode="r") as csv_file:
			...
			for row in csv_reader:
				#### FOR SWO DOCUMENT STYLED AUDITS
				# print(row)
				## Label Columns that need processing
				barcode = row[1]
				if barcode == '' or barcode == 'Barcode' or len(barcode) < 3:
					continue
				item_number = row[2].split('-', 1)[0]
				vendor_number = row[2].split('-', 2)[1] # not necesary?
				model_number = row[2].split('-', 2)[2]
				product_group = row[3]
				if '' == barcode == item_number == model_number or len(item_number) < 3:
					continue
